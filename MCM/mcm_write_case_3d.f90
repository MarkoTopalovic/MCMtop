...
IF (mod(mcm_np,2).EQ.0) THEN
   ! EVEN
   DO i = 1,mcm_np,2
      write(UNIT=32,fmt='(6(E12.5))') mcm_disp(1,i),mcm_disp(2,i),mcm_disp(3,i),mcm_disp(1,i+1),mcm_disp(2,i+1),mcm_disp(3,i+1)
      write(UNIT=35,fmt='(6(E12.5))') par(i)%v(1),par(i)%v(2),par(i)%v(3),par(i+1)%v(1),par(i+1)%v(2),par(i+1)%v(3)
      write(UNIT=36,fmt='(6(E12.5))') par(i)%a(1),par(i)%a(2),par(i)%a(3),par(i+1)%a(1),par(i+1)%a(2),par(i+1)%a(3)
   ENDDO
   !
ELSE
   ! ODD
   DO i = 1,mcm_np-1,2
      ...
      write(UNIT=36,fmt='(6(E12.5))') par(i)%a(1),par(i)%a(2),par(i)%a(3),par(i+1)%a(1),par(i+1)%a(2),par(i+1)%a(3)
   ENDDO
   write(UNIT=32,fmt='(3(E12.5))') mcm_disp(1,mcm_np),mcm_disp(2,mcm_np),mcm_disp(3,mcm_np)
   write(UNIT=35,fmt='(3(E12.5))') par(mcm_np)%v(1),par(mcm_np)%v(2),par(mcm_np)%v(3)
   write(UNIT=36,fmt='(3(E12.5))') par(mcm_np)%a(1),par(mcm_np)%a(2),par(mcm_np)%a(3)
   !
ENDIF
!
pos = 0
DO i=1,mcm_np
   ...
   ELSE
      ...
      WRITE(49,'(2X,I10)') par(i)%boundary
      WRITE(50,'(2X,I10)') par(i)%nnbr
      pos = 0
   ENDIF 
   ! 
ENDDO
	...
	do i=1,mcm_np
	   IF (par(i)%mat.EQ.j) THEN
          write(UNIT=39,fmt='(6(E12.5))') par(i)%sigma(1,1), par(i)%sigma(2,2), &
                                          par(i)%sigma(3,3), par(i)%sigma(1,2), &
										  par(i)%sigma(2,3), par(i)%sigma(1,3)
          !
		  pos = pos + 1
          IF (pos.LT.6) THEN
             ! write efps - ensight - tom - 16-9-2002
...
!
end SUBROUTINE mcm_write_case_3d